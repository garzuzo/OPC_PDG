  ...
  watch: {
    filter: {
      ...
      handler(filter){
        for (var i = 0; i < 2; i++) {
        var msg = "";
        if (filter.topicGeneral != 0) {
          msg += `topic_general=${filter.topicGeneral}&`;
        }
        if (filter.sex != "") {
        ...
        if(filter.minDate != ""){
          msg += `min_date=${filter.minDate}&`;
        }
        if(filter.maxDate != ""){
          msg += `max_date=${filter.maxDate}&`;
        }
        //comuna
        if (i == 0) {
          msg += `type_filter=comuna`;
      ...
      return feature => {
        ...
        if (comuna_corregimiento < 23) {
          if (respComuna[comuna_corregimiento] <= 0.10) {
            ...
            respComuna[comuna_corregimiento] > 0.25 &&
            respComuna[comuna_corregimiento] <= 0.5
          ) {
            fillColor = "#129793";
          } else if (
            respComuna[comuna_corregimiento] > 0.5 &&
            respComuna[comuna_corregimiento] <= 0.75
          ) {
            fillColor = "#2f5887";
          } else if (
            respComuna[comuna_corregimiento] > 0.75 &&
            respComuna[comuna_corregimiento] <= 1.0
          ) {
            fillColor = "#2b374b";
          }
        } else {
          if (respCorregimiento[comuna_corregimiento] <= 0.10) {
            ...
            respCorregimiento[comuna_corregimiento] > 0.25 &&
            respCorregimiento[comuna_corregimiento] <= 0.5
          ) {
            fillColor = "#129793";
          } else if (
            respCorregimiento[comuna_corregimiento] > 0.5 &&
            respCorregimiento[comuna_corregimiento] <= 0.75
          ) {
            fillColor = "#2f5887";
          } else if (
            respCorregimiento[comuna_corregimiento] > 0.75 &&
            respCorregimiento[comuna_corregimiento] <= 1.0
          ) {
            fillColor = "#2b374b";
          }
        }
